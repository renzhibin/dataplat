...
def cmdArgsDef():
    ...
    arg_parser.add_argument('-job_name', '--job_name', help='[help]job_name', required=True)
    arg_parser.add_argument('-job_time', '--job_time_str', help='[help]job_time_str', required=True)
    arg_parser.add_argument('-dispatch_time', '--dispatch_time', help='[help]dispatch_time', required=True)
    arg_parser.add_argument('-job_run_id', '--job_run_id', help='[help]job_run_id', required=True)
    arg_parser.add_argument('-submitter', '--submitter', help='[help]submitter', required=True)
    arg_parser.add_argument('-state', '--state', help='[help]state', required=True)
    arg_parser.add_argument('-priority', '--priority', help='[help]ext_json', required=True)
    arg_parser.add_argument('-ext_json', '--ext_json', help='[help]ext_json', required=False, default=None)
    arg_parser.add_argument('-di_run_str', '--di_run_str', help='[help]run_str', required=False, default='')
    args = arg_parser.parse_args()

    return args

def save_run_log(job_name, job_time, dispatch_time, job_run_id, submitter, priority, state, ext_json):
    mmsMysql = MmsMysql(mmsMysqlConf.MMS_DB_META_SLAVE)
    conn = mmsMysql.get_conn()
    cur = mmsMysql.get_cur()
    app_name_conf = job_name.split('.')
    stat_date = job_time
    schedule_level = 'day'
    if len(job_time) == 13:
        stat_date = job_time + ':00'
        schedule_level = 'hour'
    step = ext_json
    if step:
    ...
    if state in [0,1,2,6]:
        ...
        creater = submitter
    sql = "insert into mms_run_log(id,app_name,stat_date,status,create_time,run_module,step,creater,priority,schedule_level,task_queue,submitter,conf_name) " \
          "values(%s,'%s','%s',%s,'%s','%s','%s','%s',%s,'%s','%s','%s','%s') ON DUPLICATE KEY UPDATE status=%s" % (job_run_id, app_name_conf[0], stat_date, conf.READY, dispatch_time, app_name_conf[1] + '.' + app_name_conf[2], step, creater, priority, schedule_level, 'inf', submitter, 'inf01', conf.READY)
    cur = conn.cursor()
    cur.execute(sql)
    conn.commit()
    cur.close()
...
def error_exit(err_code, err_msg):
    ...
    if err_code == conf.FAILED:
        ...
        if ip_href.has_key(local_ip):
            ...
            raise Exception('raise execption')
...
def run_hql(hql, cat, group,set_str,status=conf.WAITING):
    try:
        ...
        if step.strip().lower() == 'all' or step.strip().lower() == 'hive':
            ...
            if True:
                ...
                if app_name in hive11_list:
                    cmd = "hive -f %s >%s" % (tmp_hql_file, out_f)
                else:
                    cmd = "hive -f %s >%s" % (tmp_hql_file, out_f)
...
if __name__ == '__main__':
    ...
    cmd_args = cmdArgsDef()
    save_run_log(cmd_args.job_name, cmd_args.job_time_str, cmd_args.dispatch_time, cmd_args.job_run_id, cmd_args.submitter, cmd_args.priority, cmd_args.state, cmd_args.ext_json)
    queue_id = cmd_args.job_run_id
    checkFlag=False
    ...
    if checkFlag==True:
        ...
        '''try:
            ...
            # update_queue_status(conf.READY)
            sys.exit(-1)'''
        run_hql(hql, cat, group,cmd_args.di_run_str,status)
