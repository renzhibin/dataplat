...
class VisualController extends Controller
{
    ...
    function actionIndex(){
        ...
        $menuConf[]=array('name'=>'报表注释','url'=>'/project/comments');
        $menuConf[]=array('name'=>'项目时间线','url'=>'/addition/showtimeline');
        if($id!='0'){
            $params = array();
        ...
        $tplArr['is_super'] = $this->objuser->isSuper();
        $tplArr['commonMenu'] = $menuInfo['commonMenu'];//常用工具等
        $tplArr['menuTitle'] =$menuInfo['menuTitle'];
        $tplArr['isCollect']=$this->objuser->isFavorites($id);
        //TODO
        ...
        if($id>0 && $customFlag === false){
            if(!empty($menu_id) ){
                if(!empty($nowMenuinfo)){
                    foreach($nowMenuinfo['table'] as $table_info){
                        if($table_info['id']==$id){
                            $reportauth=true;
                            break;
                        }
        ...
        if (isset($tplArr['confArr']['type']) && $tplArr['confArr']['type'] == 9) {
            $time                     = time();
            $md5                      = MD5("CfQNGOKatc1s6TX2rT7OeG8FHdNJ3KvP" . $time . 'dt..com');
        ...
        if ($this->isMobile()) {
            $this->render('visual/index.wap.tpl',$tplArr);
        } else {
    ...
    function actionGetData()
    {
        ...
        $time = time() - $time1;
        file_put_contents('/tmp/ttt.11', $time . "\n", FILE_APPEND);
        ...
        if($params['type'] !=2 ){
            ...
            if (!empty($data['data'])) {
                $filename = $_REQUEST['report_title']
                    . date("Ymd", strtotime($params['date'])) . "_"
                    . date("Ymd", strtotime($params['edate'])) . '_' . $fileNamePrefix . ".xls";
    ...
    function actionRun_task(){
        ...
        $data=$_REQUEST;
        $res=$this->objFackcube->get_fakecube('run_task',$_REQUEST);
