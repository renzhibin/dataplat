            ...
            <tbody>
              {/foreach from =$list item= item key=key/}
                <tr class="gradeX">
                  ...
                  <td><a target="_blank" href={/$item.real_log/}>日志</a></td>
                </tr>
              {//foreach/}
            </tbody>
          </table> 
...
<div id='addbox'>
  <table class="table table-bordered table-condensed" style='margin:10px 0px 0px 0px'>
    <tr>
       <td style='text-align:right; width:30%; color: red;'>注意</td>
       <td style='color: red;'>
           开始结束日期将被格式化为【年-月-日 小时:00:00】格式<br>
           只支持完整小时刷数，选择的分钟将被忽略
       </td>
    </tr>
    <tr>
       ...
       buttons: [{
           ...
           handler:function(){
              ...
              if(runinfo.end_time ==''){
               ...
               $('body').mask('正在操作...');
               $.post('/project/saveRealRun', {'runinfo':runinfo},function(data){
                 $('body').unmask();
                 if(data.status ==0){
                      $.messager.alert('提示',data.msg,'info');
                      $('#addbox').dialog('close');
                      window.location.href ='/project/realrunlist?project=' + runinfo.project;
                 }else{
                      $.messager.alert('提示',data.msg,'info');
                 }
               }, 'json');
