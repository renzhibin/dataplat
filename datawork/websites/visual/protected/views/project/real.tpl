...
<div class='tablehtml' style="display:none">
    ...
    <table class='table table-bordered table-condensed'>
        <tr class="storetypebox" style="display:none">
            ...
            <td>
                <select name="hql_type" class="hql_type">
                    <option value="3">实时类</option>
                </select>
            </td>
        </tr>
        <tr class="storetypebox storebox" style="display:none">
            <td>存储类型</td>
            <td><select name="storetype" class="storetype"><option value="5">mysql</option></select></td>
        </tr>
        <tr>
...
<div class='hqlHtml' style="display:none">
    <div class ='hqlInfo'>
        ...
        <h4>源数据库</h4>
        <select class="source_db" name="source_db" style="width:200px;border-color:#ccc; height:24px;">
            {/foreach $source_db as $key => $value/}
            <option value="{/$value.key/}">{/$value.value/}</option>
            {//foreach/}
        </select>
        <b style='color:red'>*</b>
        <h4>目标数据库</h4>
        <select class="target_db" name="target_db" style="width:200px;border-color:#ccc; height:24px;">
            {/foreach $target_db as $key => $value/}
            <option value="{/$value.key/}">{/$value.value/}</option>
            {//foreach/}
        </select>
        <b style='color:red'>*</b>
        <h4>目标数据表</h4>
        <input type="input" name='target_table' style='width:300px'/> <b style='color:red'>*</b>
        <h4>SQL</h4>
        ...
        <h4>SQL执行设置</h4>
        <button class='btn btn-primary hqlAnalyse' style='display:block;margin-top:3px'>SQL解析</button><span id="contain" style='display:none' >正在解析，请稍后。。。</span>
        <h4>解析信息</h4>
