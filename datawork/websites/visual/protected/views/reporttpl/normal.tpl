...
<div style='width:100%;margin:auto' id='canvas_down'>
    ...
    <div id='search'></div>
    <div id='chartTpl' class="bg-transparent"></div>
    <!-- chart 折叠 -->
...
<script type="text/javascript">
    ...
    if( params.basereport.cn_name  !=undefined){
       // $('title').text(params.basereport.cn_name +"-星图BI平台") ;
       // 兼容ie8 设置title
        document.title=params.basereport.cn_name +"-星图BI平台";
    }
    {//if/}
                ...
                if(len > 0){
                    for(var i=0; i<len; i++){
                        ...
                        tag += "<div class='configBox'>" + 
                                ...
                                "</div>";
                        //下载报表名称
                        downstr += "<form method='post' action='/visual/downData' id='downData" + i + "' class='downData' title='" + title_name + "'>" + 
                                        "<input type='hidden' name='downConfig' value='" 
                                            + encodeURIComponent(JSON.stringify(params.tablelist[i])) + 
                                        "'/>" +
                                        "<input type='hidden' name='report_title' value='" + title_name + "'/>" +
                                    "</form>";
                        isChecked = i === 0 ? "checked='checked'" : '';
                        downtablestr += '<input type="checkbox" name="tablename'+i+'" value="'+title_name+'"' + isChecked + ' /><lable for="tablename'+i+'">'+title_name+'</lable><br/>';
                    }
                } else {
