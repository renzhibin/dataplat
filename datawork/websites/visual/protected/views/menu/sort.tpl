        ...
        <div id="content" class="content-top">
            ...
            <div class='container'>
                <div style='position:relative'>
                    ...
                    <a class="pull-right" href='/menu/index'>返回列表页</a><br>
                    <span class='tipinfo' style='display:none'>(拖拽表格行进行排序)</span>
                    <div class="changebox"></div>
                    <button class="sortMenu btn btn-primary btn-sm" style="display:none" >排序保存</button>
                </div>
            </div>
        </div>
    </div>
...
<script type="text/javascript">
    $(function(){
        $('body').on('change','.menuChange',function(){
            ...
            $.get('/menu/getsecondmenu', {'first_menu': first_menu},
                function(data){
                    if(data.status ==0){
        ...
        $("body").on("click",'.sortMenu',function(){
            //获取信息
            var obj = [];
            var first_menu = $('.menuChange').select2('val');
            ...
            //$('body').mask();
            $.post('/menu/savesort', {'sortinfo':obj,'first_menu':first_menu},function(data){
                //$('body').unmask();
                if(data.status ==0){
                    $.messager.alert('提示',data.msg,'info');
                }else{
