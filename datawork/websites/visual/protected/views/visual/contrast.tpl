...
<div style='height:50px'></div> <!-- 以前值为10px -->
<div class='container'>
     ...
     <div class='row search_style' style="margin: 0px">
      <form action='' method='post' id='contrastform'>
       ...
       <div class='pull-right' style='padding:8px 10px 0px 0px'>
           ...
           {//if/}
           <span class='savereportcustom' style='cursor:pointer;padding-left:7px'>添加为收藏</span>
       </div>
      </form>
     </div>
    </div>
...
<div id='addboxcustom'>
    <table class="table table-bordered table-condensed" style='margin:10px 0px 0px 0px'>
        <tr>
            <td style='text-align:right;width:30%'>收藏报表名称<b style='color:red'>*</b></td>
            <td>
                <input type="text" name="cn_name" style="width: 300px"/><br>
                <span class="tipinfoother">(报表名必须是中英文、数字、小括号或者下划线且不超过15个字符!)</span>
                <input type ='hidden' name="params" value="{/$params/}"/>
    ...
    $(function(){
        ...
        $('#addboxcustom').dialog({
            ...
            buttons: [{
                ...
                handler:function(){
                    var config   = $('input[name=params]').val();
                    var cn_name  = $("#addboxcustom").find('input[name=cn_name]').val();
                    if($.trim(cn_name) ==''){
                        $.messager.alert('提示','请填写收藏名称','info');
                        return;
                    }
                    var basereport ={};
                    var timereport ={};
                    ...
                    timereport.offset =1;
                    timereport.shortcut =[7,30];
                    var url = '/report/savereportcustom';
                    ...
                    $.post(url, {'params':JSON.stringify(params)},function(data){
                        if(data.status==0){
                            $('#addboxcustom').dialog('close');
                            var tipstr ="<h5>操作成功！</h5><br/>";
                            tipstr +='<a target="_blank" href="/report/showreportcustom/'+data.data+'">查看报表</a><br/>';
                            $.messager.alert('提示',tipstr,'info');
                            $('#addbox').dialog('close');
                        }else{
                            $.messager.alert('提示',data.msg,'warning');
       ...
       $('.datepicker').on('changeDate',function(ev){
        ...
        $('.savereportcustom').on('click',function(){
            $('#addboxcustom').dialog('open');
        });
