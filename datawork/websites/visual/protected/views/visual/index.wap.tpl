{/include file="wapTest/layouts/header.tpl"/}
{/include file="wapTest/layouts/script.tpl"/}
<link rel="stylesheet" type="text/css" href="/assets/css/wapTest/index.css?version={/$version/}">
<link rel="stylesheet" type="text/css" href="/assets/css/wapTest/searchtime.css?version={/$version/}">

<script src="/assets/js/wapTest/filter.js?version={/$version/}"></script>
<style type="text/css">
  ...
  <div id ='right'>
      {/if $allcontent neq '' /}
        <div id="all-content" class="all-content" >
            {/else/}
            {/include file='wapTest/layouts/menu.tpl'/}
    ...
    {/if $reportauth eq 'true' /}
      {/if $menu_id eq 0  /}
        {/if $collect|@count eq 0  && $id eq 0  /}
          ...
          <div class='h4show'>
            <h4 >星图BI平台</h4>
            <p>集数据报表配置、多维分析、数据调度管理、自动邮件管理等<br>多种功能于一体的通用BI平台</p>
        ...
        {/else/}
          ...
          {/else/}
            ...
            <div  class="rightreport" style="margin-left:-10px">
                {/if $confArr['type'] == '4'/}
                {/include file='wapTest/tooltpl/showreport.tpl'/}
                {/elseif $confArr['type'] == '9'/}
                {/include file='wapTest/reporttpl/openurl.tpl'/}
                {/else/}
                {/include file='wapTest/reporttpl/common.tpl'/}
                {//if/}
            </div>
          {//if/}
        {//if/}
          ...
          //点击tab，设置title，外链等页面需要设置，所以统一设置了
           titleObj.each(function(){
              if($(this).children('li').hasClass('active')){
                  document.title= $.trim($(this).children('.active').text())+ "-星图BI平台";
              }else{
                  document.title=$.trim($(this).children().eq(0).text())+"-星图BI平台";
              }
          });
      }
...
{/include file='wapTest/layouts/menujs.tpl'/}
{/include file='wapTest/visualtpl/list.tpl'/}
{/include file='wapTest/visualtpl/chart.tpl'/}
{/include file="wapTest/layouts/footer.tpl"/}
