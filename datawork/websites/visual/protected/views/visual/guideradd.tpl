...
<div>
  ...
  <div id='right'>
                ...
                <tr class='firstMenu'>
                  ...
                  <td>
                    <select name='parent_id' class='inputall'>
                      ...
                      <a href="#" class="btn btn-primary btn-sm active addToolClassifyName" role="button">添加分类名称</a>
                  </td>
                </tr>
                <tr class='secondMenu'>
    ...
    <!--添加工具分类菜单-->
    <div id="addToolClassifyNameModal" style='display:none' >
        <table class='table table-condensed table-bordered'>
            <tr>
                <td class='table_left'>分类名称<b style='color:red'>*</b></td>
                <td>
                    <input type='text' name='name' class='inputall'/>
                </td>
            </tr>
            <tr>
                <td class='table_left'>内容<b style='color:red'>*</b></td>
                <td>
                    <input type='text' name='content' class='inputall'/>
                </td>
            </tr>
...
<script type="text/javascript">
    $(function(){
        //添加工具分类菜单
        $("body").on("click",'.addToolClassifyName',function(e){
            open("addToolClassifyNameModal",e);
        });

        $('#addToolClassifyNameModal').show().dialog({
            title: '添加分类名称',
            width: 450,
            //height:'',
            closed: true,
            ...
            buttons: [{
                ...
                iconCls:'icon-ok',
                handler:function(){
                    var $form = $('#addToolClassifyNameModal');
                    var name = $form.find('input[name=name]').val();
                    var content = $form.find('input[name=content]').val();
                    $.get('/visual/addToolClassifyName', {'name':name, 'content':content},function(data){
                        $('body').unmask();
                        if(data.status ==0){
                            location.reload();
                        }else{
                ...
                text:'取消',
                handler:function(){
                    $('#addToolClassifyNameModal').dialog('close');
                }
            }]
        });
