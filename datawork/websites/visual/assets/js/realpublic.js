    ...
    tables.find('tr:not(:first)').each(function(index){
        if(index != (len-1)){
            ...
            if( name && name != '' && cn_name && cn_name != '' ){
                table = {"name":name, "cn_name":cn_name,"ischecktables":ischecktables};
                tablearr.push(table);
            } else if (name == '' && cn_name == ''){
            } else {
                if(name == ''){
...
/*    $('.container').on('focus','input[name="name"]',function(){
    ...
    $('.container').on('blur','input[name="name"]',function(){
        ...
        var checkParams = {"item_val":project_name,"hql_type":2,"store_type":store_type,"item_type":type};
        $.get('/project/checkrealprojectdata',checkParams,function(data){
                if(data.status=='0'){
    ...
    if(project.length >0 ){
        ...
        if(categories.length >0){
            for(var i =0; i < categories.length; i++){
                ...
                html += "</span>";
                html +="<button class='btn btn-primary btn-xs addHql pull-right' style='z-index:1000; position:relative'>添加SQL</button>";
                html +="<b class='closeBtn'>X</b>";
    ...
    $('body').on('click','.saveInfo',function(){
        ...
        //保存信息校验
        $.ajax({
            url : "/project/checkrealprojectdata",
            method : "get",
            data: {"item_val":project_name,"hql_type":2,"store_type":store_type,"item_type":type},
            async: false,
        ...
        switch(type){
            ...
            case 'categories':
                ...
                html += "</span> <button class='btn btn-primary btn-xs addHql pull-right'>添加SQL</button>";
                html +="<b class='closeBtn'>X</b>";
            ...
            case 'groups':
                ...
                groups.source_db = $('.rightContent').find('select[name="source_db"]').val();
                groups.target_db = $('.rightContent').find('select[name="target_db"]').val();
                groups.target_table = $('.rightContent').find('input[name="target_table"]').val();
                ...
                groups.hql_type = (project['hql_type'])?project.hql_type:3;
                if(groups.cn_name ==''){
                    alert('请填写中文名');
                    return;
                }
                ...
                if(groups.target_db != 'metric_real_data'){
                    alert('请填写目标数据库');
                    return;
                }
                */

                if(groups.target_table == ''){
                    alert('请填写目标数据表');
                    return;
                } else {
                    var alertFlag = false;
                    var sendOJb   = {};
                    sendOJb.db    = groups.target_db;
                    sendOJb.table = groups.target_table;
                    $.ajax({
                        type: "POST",
                        url: "/project/checkTargetTable",
                        data: sendOJb,
                        dataType: "json",
                        async: false,
                        success: function(data){
                            if(data.status == 0){
                                alert('数据库【' + groups.target_db + '】已存在表【' + groups.target_table +  '】');
                                alertFlag = true;
                            }
                        }
                    });
                ...
                if(localConfigHql.hql && localConfigHql.hql != groups.hql){
                    alert('您的SQL已经发生改变，请您重新SQL解析！');
                    $(".metricsBox").html('');
                    $(this).attr('disabled',true);
                    return false;
                }

                if( groups.hql.indexOf('$MINUTE') <0 ){
                    if( window.confirm('亲,没有用$MINUTE函数哦,你这样有可能每天跑的数都是一样的哦,亲，你确定吗') ){
                    }else{
                        return false;
                    }
                }

                if(groups.hql_type == "3") {
                    ...
                    if(groups.metrics.length == 0 && groups.dimensions.length == 0){
                        alert('您SQL解析的纬度、指标不能同时为空,请您重新解析');
                        return false;
                    }
    ...
    $('body').on('click','.addHql',function(){
        ...
        $('.rightContent').find('.addtitle').text('添加SQL');
        localConfigHql = {};
        ...
        $('.rightContent').find('select[name="schedule_interval"]').val("5");
        ...
        switch(type){
            ...
            case 'groups':
                ...
                if(window.location.href.indexOf("cubeeidtor") > 0){
                    ...
                    history.pushState({}, "", "?project="+project+"&id="+id+"&groupname="+groupnames);
                ...
                var schedule_interval = data.schedule_interval?data.schedule_interval:"5";
                ...
                $rightContent.find('select[name="source_db"]').val(data.source_db);
                $rightContent.find('select[name="target_db"]').val(data.target_db);
                $rightContent.find('input[name="target_table"]').val(data.target_table);
                ...
                $rightContent.find('.addtitle').text('编辑SQL');
    ...
    $('.saveConfig').click(function(){
        ...
        function saveConfig() {
            ...
            if( projectInfo !=undefined){
                ...
                $.ajax({
                    type: "POST",
                    url: "/project/saveRealProject",
                    data: sendOJb,
                    dataType: "json",
                    success: function(data){
                        if(data.status ==0){
                            alert('项目配置保存成功！');
                            window.location.href ='/project/real';
                            localtempcount = -1;
                        }else{
                            alert(data.msg);
                        }
    ...
    $('body').on('click','button.addtableBtn',function(){
        var str = '<tr><td contenteditable="true" data-type="name"></td><td contenteditable="true" data-type="cn_name"></td><td style="text-align:center"><input type="checkbox" class="ischecktables" name="ischecktables" checked></td><td><button class="btn btn-default btn-xs removetableBtn">删除</button></td></tr>';
