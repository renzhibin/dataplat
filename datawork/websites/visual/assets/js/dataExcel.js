...
dataExcel.prototype = {
      ...
      buttons: [{
        ...
        handler: function() {
          ...
          $reportgrade.find('.gradebox').find('tr').each(function() {
            ...
            var reshape_type = $("#" + _this.option.id).find('.btn_group_change').attr('data-type');
            if (is_reshape) {
                if (search.is_check == '1') {
                    $.messager.alert("提示", '选中行列转换时，过滤条件不允许多选', 'warning');
                    return;
                }
                if (!search.val) {
                    $.messager.alert("提示", '选中行列转换时，过滤条件数据不能为空', 'warning');
                    return;
                }
                if (!search.defaultsearch && parseInt(reshape_type) != 1) {
                    $.messager.alert("提示", '选中行列转换时，默认搜索值不能为空', 'warning');
                    return;
                }
          }
  ...
  createtable: function() {
    ...
    if (header.length > 0) {
      ...
      tableHtml += "<tr class='table_header' style='background: #979797 !important;'>";
      for (var i = 0; i < header.length; i++) {
        ...
        switch (data.header[i].key) {
          ...
          case 'width':
              tableHtml += "<td style='width:" + header[i].width + "'>" + header[i].name + "<br>";
              tableHtml += "<button class='btn btn-success btn-xs checked_" + data.header[i].key +
                  "'>计算</button>";
              tableHtml += "</td>";
              break;
          default:
      ...
      // 判断是否为自定义 chart
      if(numData[0] && (numData[0].type === '指标类型' || numData[0].type === '维度类型')) {
        isCustomChart = true;
      }
    ...
    if (!isCustomChart) {
      tableHtml += '<button class="btn btn-primary btn-xs addColumn">增加一行</button><input type="hidden" class="chart_conf"/>';
    }
        ...
        content:function(){
          if($('.group_box').length >0){
             return $('.group_box');
              ...
              $('body').append('<div class=group_box></div>');
              $('.group_box').append("<span><span>正常报表</span><img width=300px src='/assets/img/normal.jpg'/></span><span><span>时间-指标类型</span><img width=400px src='/assets/img/time.jpg'/></span><span><span>维度--指标类型</span><img width=300px src='/assets/img/group.jpg'/></span>");
              return $('.group_box');
          }
        }
    });
    ...
    switch (key) {
      ...
      case 'key':
        ...
        if (type == 3) {
          ...
          td += "<small class='clipBtn' data-clipboard-text='" + keyVal + "' id='clipBtn_" + item + "' title='" + keyVal +
            "' data-clipboard-target-old='clipContent_" + item + "' data=" + item + ">复制</small>";
      ...
      case 'relative_ratio':
          if (type == 1) {
              ...
              td += "<select class='ck_relative_ratio'>";
              td += "<option value='0' "
              if (keyVal == '0') {
                  td += " selected ='selected' ";
              }
      ...
      case 'indent_count':
          if (type == 1 || type == 3) {
              ...
              td += "<select class='ck_indent_count'>";
              td += "<option value='0' "
              if (keyVal == '0') {
                  td += " selected ='selected' ";
              }
  ...
  getCell: function(key, parent, type) {
    ...
    switch (key) {
      ...
      case 'sum_ratio':
        ...
        break;
      case 'relative_ratio':
        val = obj.find("select.ck_relative_ratio").val();
        break;
      case 'indent_count':
        val = obj.find("select.ck_indent_count").val();
        break;
      case 'thousand':
        if (obj.text() == '-') {
          val = "-";
  ...
  getData: function() {
    ...
    $('#' + this.option.id).find('.gradebox tr').each(function() {
      ...
      if (typeval == '维度' || typeval == '维度类型') {
        one.isgroup = 1;
      } else if (typeval == '指标' || typeval == '指标类型') {
        one.isgroup = 2;
    ...
    if (ischeck) {
      //验证行列转换
      if(pubdata.reshape_type ==1 ){
        var tmpStatus =0;
        for(var i=0; i< grade.data.length;i++){
           if( grade.data[i].key == pubdata.reshape_group  &&  grade.data[i].search.defaultsearch !=''){
              $.messager.alert('提示', '选择转换的维度搜索默认值必须为空', 'info');
              return false;
           }
           if( grade.data[i].type == '维度'  &&  grade.data[i].key != 'date' && grade.data[i].key != pubdata.reshape_group  &&  grade.data[i].search.defaultsearch =='' ){
              $.messager.alert('提示', grade.data[i].name +' 此维度不是行列转换选中的维度，搜索默认值不能为空！', 'info');
              return false;  
           }
        }
        if( parseInt(window.params.timereport.date_type) != 1){
            $.messager.alert('提示', '维度--指标类型报表时间区类型只支持单天', 'info');
            return false;
        }
  ...
  getpubData: function() {
    ...
    var data_num = $("#" + this.option.id).find('input[name="data_num"]').val();
    temppubdata['data_num'] = data_num !=''? parseInt(data_num):0;
    if (temppubdata['data_num'] > 499) {
        $.messager.alert('提示', '您设置的数值不能大于等于500', 'info');
        return false;
      }

    var _this = this;
    var reshapetag = $("#" + this.option.id).find('input[name="reshape"]');
    if (reshapetag.length > 0) {
        temppubdata['reshape'] = reshapetag.is(':checked') ? '1' : '0';
        temppubdata['reshape_type'] = $("#" + this.option.id).find('.btn_group_change').attr('data-type');
        temppubdata['reshape_group'] = $("#" + this.option.id).find('select[name=reshape_group]').val();
    }
    ...
    if (ispagesizetag.length > 0) {
      ...
      //设置报表显示时间间隔
      var isdateslicetag = $("#" + this.option.id).find('.isdateslicebox input[name="isdateslice"]');
      if (isdateslicetag.length > 0) {
          temppubdata['isdateslice'] = isdateslicetag.is(':checked') ? '1' : '0';

          var dateslice = $("#" + this.option.id).find('.dateslicebox input[name="dateslice"]').val();

          if (dateslice == '' || dateslice < 0 || dateslice % 1 !== 0) {
              $.messager.alert('提示', '请输入被1整除的整数', 'info');
              return false;
          }
          temppubdata['dateslice'] = (dateslice && dateslice > 0) ? dateslice : 1;
      }

    // console.log(temppubdata);
    return temppubdata;
  ...
  setpubData: function(pubdata) {
    ...
    var reshape = (temppubdata.reshape == 1) ? true : false;
    $("#" + this.option.id).find(' input[name="reshape"]').prop('checked',
        reshape);

    if(temppubdata.reshape_type ==1){
      $("#" + this.option.id).find('.btn_group_change').text('维度--指标类型');
      $("#" + this.option.id).find('.reshape_box').show();
      $("#" + this.option.id).find('.reshape_tips').text('（提示：选中行列转换后，列无法排序');

      var htmlstr = this.create_reshape_html(this.option.data,temppubdata.reshape_group);

      $("#" + this.option.id).find('.reshape_row').html(htmlstr);
    }else{
      $("#" + this.option.id).find('.btn_group_change').text('时间-指标类型');
      $("#" + this.option.id).find('.reshape_box').hide();
      $("#" + this.option.id).find('.reshape_tips').text('（提示：选中行列转换后，报表展示时将隐藏维度（维度必须设置搜索与即使过滤条件），并且列无法排序）');
      $("#" + this.option.id).find('.reshape_row').html('');
    }
    $("#" + this.option.id).find('.btn_group_change').attr('data-type',temppubdata.reshape_type);
    //如果报表为行列转换，不设置值
    if(!reshape){
      $("#" + this.option.id).find('.datanumbox').show();
      if(temppubdata.data_num){
         $("#" + this.option.id).find('input[name=data_num]').val(temppubdata.data_num);
      }else{
        $("#" + this.option.id).find('input[name=data_num]').val('');
      }
       
    }else{
      $("#" + this.option.id).find('.datanumbox').hide();
      $("#" + this.option.id).find('input[name=data_num]').val('');
    }
    //表格页码设置
    ...
    //获取页码设置
      var isdateslicebox = $("#" + this.option.id).find('.isdateslicebox');
      if (isdateslicebox.length == 0) {
          return false
      }

      var dateslicetag = isdateslicebox.find('input[name="dateslice"]');
      var isdateslice = ((typeof(temppubdata.isdateslice) != 'undefined' && temppubdata.isdateslice ==
      '0') || typeof(temppubdata.isdateslice) == 'undefined') ? true : false;
      var dateslice = (typeof(temppubdata.dateslice) != 'undefined') ? temppubdata.dateslice : 1;

      isdateslicebox.find('input[name="isdateslice"]').prop('checked', !isdateslice);
      //禁用的功能
      if (pubdata['dateslice_disabled'] && pubdata['dateslice_disabled'] == '1') {
          isdateslicebox.find('input[name="isdateslice"]').prop('disabled', 'true');
      }

      $(dateslicetag).numberspinner({
          min: 1,
          max: 10,
          increment: 1,
          value: dateslice
      ...
      if (!isdateslice) {
          isdateslicebox.find('.dateslicebox').show();
      } else {
          isdateslicebox.find('.dateslicebox').hide();
      }
  },
      ...
      for (var i = 0; i < header.length; i++) {
        ...
        switch (header[i].key) {
          ...
          case "expression":
            ...
            break;
          case "percent":
            columndHtml += "<input type='checkbox' class='percent_checkbox'>";
            break;
          case "thousand":
            columndHtml += "<input type='checkbox' class='thousand_checkbox'>";
            break;
          case "issearch":
          ...
          case "otherlink":
            ...
            break;
          case "fixed":
            columndHtml += "<input type='checkbox' class='fixed_checkbox'>";
            break;
          case "relative_ratio":
            columndHtml += "<select class='ck_relative_ratio'><option value='0'>不显示</option><option value='1'>环比率</option><option value='2''>环比值</option></select>";
            break;
          case "sum_ratio":
          case "indent_count":
            columndHtml += "-";
  ...
  is_fixed: function() {
    ...
    var source = $("#" + this.option.id);
    source.on('click', 'input.checked_fixed', function() {
      // alert('ssss');
      // var index = $(this).closest('tr').attr('index');
      // console.log('>>>>', index);
      var $table = $(this).closest('table');
      if ($(this).is(":checked")) {
        $table.find('td.fixed_report').each(function(i) {
      //    if (i < index) {
            $(this).find('input.fixed_checkbox')[0].checked = true;
        //  }
        });
      } else {
        $table.find('td.fixed_report').each(function(i) {
        //  if (i > index) {
            $(this).find('input.fixed_checkbox')[0].checked = false;
         // }
        });
      }
    });
  ...
  is_pagesize: function() {
    ...
    //时间是否设置
    is_dateslice: function() {
        var source = $("#" + this.option.id);
        source.on('click', 'input[name="isdateslice"]', function() {
            var obj = $(this);
            $this = $(this), $tag = $this.closest('.isdateslicebox').find('.dateslicebox');
            if ($this.is(':checked')) {
                $tag.show();
            } else {
                $tag.hide();
  ...
  create_reshape_html(data,selected){
      //获取维度数据
      var group =[];
      for(var i=0; i< data.data.length; i++){
         if(data.data[i].type =='维度' && data.data[i].key !='date'){
            group.push(data.data[i]);
         }
      }
      var html ="<select name='reshape_group'>";
      for(var j=0; j< group.length; j++){
        if(selected == group[j].key){
          html += "<option value='"+ group[j].key+"' selected='selected' >"+group[j].name+"</option>";
        }else{
          html += "<option value='"+ group[j].key+"'>"+group[j].name+"</option>";
        }
      }
      html +="</select>";
      return html
  ...
  //报表条数控制
  data_num_contrl: function() {
    var source = $("#" + this.option.id);
    var _this = this;
    source.on('click', 'input[name=reshape]', function() {
        var type = $(this).is(":checked");  
        if(type){
            $(this).text('时间-指标类型');
            source.find('input[name=data_num]').val('');
            source.find('.datanumbox').hide();
        }else{
            source.find('.datanumbox').show();
        }
    });
  },
  //维度与时间切换（行列转换）
  change_row: function() {
    var source = $("#" + this.option.id);
    var _this = this;
    source.on('click', '.btn_group_change', function() {
        var type = $(this).attr('data-type');   
        if(parseInt(type) ==1){
            $(this).attr('data-type',2);
            $(this).text('时间-指标类型');
            source.find('.reshape_box').hide();
            source.find('.reshape_tips').text('（提示：选中行列转换后，报表展示时将隐藏维度（维度必须设置搜索与即使过滤条件），并且列无法排序）');
            source.find('.reshape_row').html('');
        }else{
            $(this).attr('data-type',1);
            $(this).text('维度--指标类型');
            var data = _this.getData(false);
            //获取维度数据
            var html = _this.create_reshape_html(data);
            source.find('.reshape_box').show();
            source.find('.reshape_tips').text('（提示：选中行列转换后，列无法排序');
            source.find('.reshape_row').html(html);
        }
    });
  },
  sort_plus: function() {
  ...
  calculate_width: function () {
      var source = $("#" + this.option.id);
      source.on('click', '.checked_width', function () {
          source.find("#dragtr").find('tr').each(
              function () {
                  var keyText = $(this).find('td.name_report').find('textarea').val() || $(this).find('td.name_report')[0].innerText;
                  if (keyText) {
                      var allLength = 22; // 文字(标志)长度+22=td的长度
                      var keyLength = keyText.length;
                      var chineseNum = 0; // 汉字 长度12
                      var numberNum = 0;  // 数字 长度6
                      var upperNum = 0;   // 大写字母 长度8
                      var lowerNum = 0;   // 小写字母 长度6
                      var otherNum = 0;   // 小写字母 长度8
                      for (var i = 0; i < keyLength; i++) {
                          var keyTextItem = keyText.charAt(i);
                          if (new RegExp("[^\x00-\xff]", "g").test(keyTextItem)) {
                              chineseNum += 1;
                          } else if (new RegExp("[0-9]", "g").test(keyTextItem)) {
                              numberNum += 1;
                          } else if (new RegExp("[a-z]", "g").test(keyTextItem)) {
                              lowerNum += 1;
                          } else if (new RegExp("[A-Z]", "g").test(keyTextItem)) {
                              upperNum += 1;
                          } else {
                      ...
                      var explainText = $(this).find('td.explain_report').find('textarea').val() || $(this).find('td.explain_report')[0].innerText;
                      if (explainText) {
                          allLength += 16; // 标志长度
                      }
                      allLength += chineseNum * 12 + numberNum * 6 + upperNum * 8 + lowerNum * 6 + otherNum * 8;
                  }
                  $(this).find('td.width_report').find('input.width')[0].value = allLength > 100 ? allLength : 100;
              }
          );
      });
  },
  ...
  checkData: function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i].key != "" && data[i].key != undefined && data[i].key != null) {
        if(!new RegExp('^[a-zA-Z0-9_\.]*$', '').test(data[i].key)) {
            $.messager.alert("提示", '列key只能数字、字母、下划线、点', 'warning');
            return false;
        }
      }
      ...
      if (is_reshape) {
        if (data[i].key == 'date') {
          if (data[i].sort != 'desc') {
              $.messager.alert("提示", '选中行列转换时，时间维度默认排序必须为降序', 'warning');
              return false;
          }
        }
        if (data[i].type == '维度' && data[i].key != 'date') {
          var is_search = data[i].issearch;
          var filter = data[i].filter;
          var reshape_type = $("#" + this.option.id).find('.btn_group_change').attr('data-type');
          if (is_search instanceof Object) {
              if (is_search.is_accurate != '1') {
                  $.messager.alert("提示", '选中行列转换时，匹配类型必须为精确匹配', 'warning');
                  return false;
              }
              if(parseInt(reshape_type) ==2 ){
                var search = data[i].search;
                if (search.defaultsearch == undefined || search.defaultsearch == '') {
                    $.messager.alert("提示", '选中行列转换时，即使过滤信息必须配置', 'warning');
                    return false;
                }
              }
            ...
            if(parseInt(reshape_type) ==2 ){
              if (filter.op != '=') {
                  $.messager.alert("提示", '选中行列转换时，过滤条件操作符必须选择"＝"', 'warning');
                  return false;
              }
              if (filter.value == '') {
                  $.messager.alert("提示", '选中行列转换时，过滤条件值必须填写', 'warning');
                  return false;
              }
            }
          }else {
              $.messager.alert("提示", '选中行列转换时，维度除时间外搜索或过滤条件必须选中一个', 'warning');
              return false;
          }
        }
      }
    ...
    var allUdcKey = [];
    var errorMessage = '';
    for (var m = 0; m < udcLine.length; ++m) {
        if (/>>|->/g.test(udcLine[m][1])) {
            continue;
        }
        udcName = udcLine[m][0];
        allUdcKey = udcLine[m][1].match(/[a-zA-Z0-9_]*\.[a-zA-Z0-9_]*\.[a-zA-Z0-9_]*/g);
        for (var n = 0; allUdcKey !== null && allUdcKey !== undefined && n < allUdcKey.length; ++n) {
          if($.inArray(allUdcKey[n], commonLine) < 0) {
              errorMessage += (allUdcKey[n] + '<br>');
          }
        }
        if(errorMessage.length > 0) {
          $.messager.alert("提示", 'UDC列【' + udcName + '】指标异常<br>异常指标：<br>' + errorMessage + '', 'warning');
          return false;
        }
    }
    if (converge.length > 0) {
