...
$(function() {
  ...
  chartSource.select(function(obj, data) {
    ...
    $("#chartreport").find('button[data-option=customSource]').show();
  ...
  // checkChartSql 参数为回调函数 先进入 datasource.checkSql 处理完成后在执行参数函数
  chartSource.checkChartSql(function(obj, data) {
      data.type = 8;
      obj.hide();
      $("#chartreport").find('button[data-option=dataSource]').show();
      $("#chartreport").find('button[data-option=customSource]').show();
      ...
      var defaults = JSON.parse(JSON.stringify(chartDefault));
      defaults.group = data.group_custom.join();
      defaults.metric = data.metric_custom.join();
      defaults.sql = data.sql;
      defaults.sqldata = data.sqldata;
      defaults.customsql_start = data.customsql_start;
      if (custorType == 'cursor_line') {
          defaults.header.push({
              key: "xaxis",
              name: "自定义x轴",
      ...
      if (chartconf) {
          ...
          var srcchart = chartSource.getMte(chartInfo.group);
          var newchart = chartSource.getMte(data.group);
          if (srcchart.length != newchart.length && chartInfo.udcconf != '0' && chartInfo.udcconf != undefined && chartInfo.udcconf != '') {
              $.messager.confirm('提示', '您选择的维度有变化，是否清除udc信息', function(r) {
                  if (r) {
                      defaults.data = getDataTable(data, defaults.header);
                      chartHideSet(chartconf, defaults);
      ...
      $("#chartreport").find('#tableBox').show();
      // 屏蔽 UDC 行
      $("#chartreport").find('#tableBox').find('.addColumn').hide();
    });
  //数据源选择按钮 事件处理 (表格)
  ...
  tableSource.checkSql(function(obj, data) {
    ...
    if (gradeconf != '') {
      ...
      sqlDefault.pubdata.dateslice = gradejson.pubdata.dateslice;
      //保存上次输入的内容表格数据
  ...
  $('#chartreport').show().dialog({
    ...
    buttons: [{
      ...
      handler: function() {
        ...
        expObj.xAxisTime = $('#chartreport').find('select[name=xAxisTime]').val();
      ...
      text: '取消',
      handler: function() {
        if(window.editcode_chart) {
          window.editcode_chart.setValue('');
        }
        $('#chartreport').dialog('close');
      }
    }],
    ...
    buttons: [{
      ...
      handler: function() {
        ...
        if (window.tables == undefined || (window.tables && !window.tables[index])) {
          ...
          var tag =
            "<div  data-index=" + index + " class='configBox' ><div class='tabletitle'></div><div class='filter' index='" +
            index + "'></div>" +
        ...
        params.tablelist.map((item,index) => {
          window.tables[index].table = item;
        });
        var nodeName= $('.tablelist .configBox:last')[0].nodeName;
        /**
         * 新建的时候才执行
         */
        if(nodeName == 'DIV'){
          ...
          setTimeout(() => {
            var outerHTML = $('.tablelist .configBox:last')[0].outerHTML;
            var liHtml = `<li ${outerHTML.slice(5, -6)}</li>`;
            $('.tablelist .configBox:last')[0].outerHTML = liHtml;
            $('.tablelist .configBox:last')[0].innerHtml = liHtml;
          }, 1000);
        }else{
        // var copy= deepClone(window.tables)
        var copy = []
          var copy = _.cloneDeep(window.tables);
        // $.extend(true,copy, window.tables);
          // debugger
          copy.map((item,index) => {
            window.tables[index].mylocaltable = copy[aryIndex[index]].mylocaltable;
            window.tables[index].boxtag = copy[aryIndex[index]].boxtag;
            window.tables[index].searchtag = copy[aryIndex[index]].searchtag;
            window.tables[index].titletag = copy[aryIndex[index]].titletag;
          })
          params.tablelist.map((item,index) => {
            window.tables[index].init();
          });
  ...
  $("body").on('click', '.btn', function(e) {
    ...
    sorttablelist();
    sortIndex();
    /*if (e.currentTarget.className.indexOf('addColumn') < 0) {
        $("html,body").animate({scrollTop: 0}, 200); // 回到页面顶端便于填写内容
    }*/
    ...
    switch (strid) {
      ...
      case 'dataSource':
        ...
        $("#chartreport").find('.xAxis_time_box').hide();
        $("#chartreport").next().hide();
        $("#chartreport").find('#sourcebox').show();
        $("#chartreport").find('#sourcebox').find('.custombox').hide();
        $("#chartreport").find('#sourcebox').find('.datasourcebox').show();
        //$(this).hide();
      ...
      case 'customSource':
          ...
          */
          ...
          if('cursor_line' == $("#chartreport").find('select[name=chartType]').val()) {
            $("#chartreport").find('.xAxis_time_box').show();
          } else {
            $("#chartreport").find('.xAxis_time_box').hide();
          }
          ...
          $("#chartreport").next().hide();
          $("#chartreport").find('#sourcebox').show();
          $("#chartreport").find('#sourcebox').find('.datasourcebox').hide();
          $("#chartreport").find('#sourcebox').find('.custombox').show();
          $("#chartreport").find('#sourcebox').find('.customsql_startbox').hide();
          $("#chartreport").find('#tableBox').hide();

          // 修正sql编辑页面需要点击一下才能显示正确sql问题
          if(window.editcode_chart) {
            window.editcode_chart.setValue(window.editcode_chart.getValue());
          }
      ...
      case 'reportgrade':
        ...
        if (isEmptyObject(params.tablelist)) {
          ...
          if (params.tablelist.length > 1) {
            ...
            //params.tablelist[0].grade['pubdata']['dateslice_disabled'] = "1";
          } else {
            params.tablelist[index].grade['pubdata']['pagesize_disabled'] = "0";
            //params.tablelist[index].grade['pubdata']['dateslice_disabled'] = "0";
          }
  ...
  $('body').on('click', '.addsource', function() {
    ...
    // 修正sql编辑页面需要点击一下才能显示正确sql问题
    if(window.editcode) {
      window.editcode.setValue(window.editcode.getValue());
    }
  ...
  $('body').on('click', '.chartlist .chartedit', function(e) {
    ...
    if(tmpChart.grade && tmpChart.grade.sql) {
      $("#chartreport").find('.xAxis_time_box').show();
    } else {
      $("#chartreport").find('.xAxis_time_box').hide();
    }
  ...
  $('body').on('click', '.addTable', function(e) {
    ...
    if (index >= 10) {
      $.messager.alert('提示', '最多只能添加9个主副表格', 'warning');
      $('#addTableForMulti').hide();
      return false;
    }
    $('#reportgrade').next().hide();
  ...
  $('body').on('click', '.deleteTable', function() {
    ...
    $.messager.confirm('提示', '确定删除吗？', function(r) {
      /**
       * 先排序 删除完之后 直接将data-index 重新置位 0 ，1，2，3顺序
       */
      getIndex();
      sorttablelist();
      if (r) {
  ...
  const getIndex= () => {
    aryIndex = [];
    $('.tablelist .configBox').map((index,item)=>{
      aryIndex.push(item.getAttribute('data-index'))
    });
    return aryIndex;
  }
  /**
  ...
  /**
   * 重置 params.tablelist 顺序
   */
  const sorttablelist = () => {
    const copytablelist = Object.assign([],params.tablelist);
    copytablelist.map((item,index) => {
      params.tablelist[index] = copytablelist[aryIndex[index]];
      params.tablelist[index].master = index;
    });
  } 
