...
dataSource.prototype = {
	...
	init:function(){
        ...
        // 自定义chart的 ace 初始化
        if(this.option.id === 'sourcebox') {
            $custombox.find('.editcode').attr('id','editcode2');
            window.editcode_chart = ace.edit("editcode2");
            editcode_chart.setTheme("ace/theme/tomorrow_night_eighties");
            editcode_chart.session.setMode("ace/mode/sql");
            editcode_chart.setAutoScrollEditorIntoView(true);
            editcode_chart.setOption("minLines", 10);
            editcode_chart.setOption("maxLines", 20);
        }
	...
	setSource:function(params){
        ...
        // 自定义 chart
        if(this.option.id === 'sourcebox' && params.grade && params.grade.sql && window.editcode_chart) {
            window.editcode_chart.setValue(params.grade.sql);
        } else {
	...
	// chart 校验sql
    checkChartSql:function(sucesscallback){
        var source = $("#"+this.option.id);
        var that = this;
        source.off('click','button.checksql');
        source.on('click','button.checksql',function(){
			// 点击 校验 SQL 时，清理普通图表的配置
            that.clear();
            var $sqlerror = $(this).next('.sql_error');
            $sqlerror.hide();

            var sql = window.editcode_chart.getValue();
            if(sql == ''){
                $sqlerror.text('提示：请输入sql').show();
                return false;
            }
            var date = '2014-01-01', edate = getBeforeDate(0);
                ...
                success: function (result) {
                    if(result.status == 0){
                        if(result.data.data!=null && result.data.data && result.data.data.length>0){
                            ...
                            getdata['customsql_start'] = date;
                            getdata['group_custom'] = [];
                            getdata['metric_custom'] = [];
                            for (var item in colums) {
                                if (colums[item]['type'] === 'dim') {
                                    getdata['group_custom'].push(item);
                                } else if (colums[item]['type'] === 'metric') {
                                    getdata['metric_custom'].push(item);
                                }
                            }
