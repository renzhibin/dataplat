...
Table.prototype = {
						...
						if (tempurlsearch) {
							for (var p in tempurlsearch) {
								if (tempurlsearch[p].key == onesearch.key) {
									onesearch.defaultsearch = tempurlsearch[p].val[0];
								}
							}
						}
			...
			if (newSearch.length > 0) {
				...
				for (var i = 0; i < newSearch.length; i++) {
					if (newSearch[i].isadd == 1 && newSearch[i].value != '') {
						$(_this.searchtag).find('select[name="' + newSearch[i].key + '"]').select2('val', newSearch[
							i].value);
					}
				}
			}
				...
				$(this).children('td').each(function(td_index) {
					...
					if ($k_obj.group.split(',').indexOf($k_obj.showthis) == -1 && $(this).attr('field') !=
						...
						$(this).html("<div class='compros' style='width:" + $width +
							"px' ><span class='data_name " + oldClass + "'>" + $oldhtml + "</span>" +
	...
	dealContrst: function(result, fakecube) {
		...
		if (fakecube.master == null || fakecube.master == 0) {
			...
			$('.tablelist .configBox:eq(' + $master + ') .datagrid-view2 .datagrid-body tr').each(function(
				...
				$('.tablelist .configBox:eq(' + $master + ') .datagrid-view2 .datagrid-body tr:eq(' +
					tr_index + ')').children('td').on('mouseover', function(e) {
					var _this = $(this);
					$oldhtml = _this.children('div').html();
					$width = _this.width();
					//组装维度
					...
					if (_this.find('.data_name').length == 0) {
						...
						if ($k_obj.group.split(',').indexOf($k_obj.showthis) == -1) {
							...
							_this.html("<div class='compros'><span class='data_name " + oldClass + "'>" + $oldhtml +
								"</span>" +
	...
	createCutomkey: function(params, table) {
        ...
        if (table.grade.pubdata.reshape == 1) {
            tag +=
                '<li role="presentation" class="active my-nav-customtitle closed"><span class="web-filter" style="position: absolute;text-align: center;top: -1px;padding-top:4px;padding-bottom:2px;right: 10px;width: 55px;background:#F6F6F6;color: #999;border: 1px solid #ddd;">查询</span>';
        } else {
            tag +=
                '<li role="presentation" class="active my-nav-customtitle closed"><span class="web-filter" style="position: absolute;text-align: center;top: -1px;padding-top:4px;padding-bottom:2px;right: 90px;width: 55px;background:#F6F6F6;color: #999;border: 1px solid #ddd;">查询</span>';
        }
		if (table.grade.pubdata.reshape != 1) {
            tag += '<a href="javasript:void(0)" class="customtitle" style="margin: 0;">自定义显示</a>';
        }
