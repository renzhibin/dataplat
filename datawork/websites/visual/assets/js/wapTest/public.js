...
function createChart(data){
    ...
    switch(data.chart.type ){
      case 'funnel':
        data.tooltip.formatter = function(params){
            ...
            return   params[1] +": "+ params.data.funnel + "("+params[2]+ "%)<br>相对上个环节:"+params.data.layter+"%";
        }
      ...
      case  'pie':
            //var myChart = echarts.init(document.getElementById(data.chart.renderTo),'shine');
            data.tooltip.formatter = function(params){
              return  params[0] +"<br/>"+ params.data.detialname +":"+ params[2] + " ("+params[3]+"%)";
            }
           //myChart.setOption(data);
        break;
      default:
        ...
        data.tooltip.formatter = function(params){
            ...
            for(var j =0; j<  arr.length; j++){
                if(arr[j].y != undefined){
                  ...
                  strArr.push( arr[j].name +":"+ arr[j].y );
                }else{
    ...
    if ($('.muneIcon').css('display')!='none'){
        if($('#chartTpl').children()){
          if(window.tableAjaxFlag && window.sendChartFlag){
   ...
   $('body').on('click','.sumTotal',function(){
         ...
         if(timeInterval == undefined){
          ...
          success: function(data){
             ...
             str += "</table>";
               var boxStr ="<div id='sumBox' style='height:300px; overflow:auto'><div>";
              if ($("#sumBox").length >0){
                  var obj = $(str);
                $("#sumBox").html("");
                $("#sumBox").append(str);
