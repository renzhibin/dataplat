...
function main_subtabulation(type, isaddmeter, defaults) {
  ...
  defaults['pubdata']['isdateslice'] = 0;
  defaults['pubdata']['dateslice_disabled'] = 0;
  return defaults;
}
...
function getDataTable(source, header) {
  ...
  if (source.type && source.type == '8' && source.sqldata) {
    ...
    var dim_custom = source.group_custom || [];
    var metric_custom = source.metric_custom || [];
    for (var item in data) {
      if (data[item].key) {
        var key = data[item].key;
        if (in_array(key, dim_custom)) {
          data[item]['type'] = '维度类型';
        } else if (in_array(key, metric_custom)) {
          data[item]['type'] = '指标类型';
        }
      }
    }
...
function getDefaultData(type) {
  ...
  switch (parseInt(type)) {
    ...
    case 7:
      ...
      break;
    default:
      tableDefault.header.splice(13, 0,
          /*{ // 后期占比功能下掉，使用UDC实现
            key: "sum_ratio",
            "name": "占比",
            width: "4%"
...
function chartSrc(allConf, sort) {
  ...
  chartObj.find('select[name=xAxisTime]').select2('val', allConf.chartconf[0].xAxisTime == undefined ? 'filter_not' : allConf.chartconf[0].xAxisTime);
  //单位设置
