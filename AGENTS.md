...
- **PHP 8.3 compatibility**: Yii 1.x uses dynamic properties deprecated in PHP 8.2+. The `index.php` suppresses `E_DEPRECATED` errors for compatibility. This is a runtime environment fix, not a functional change.
...
- **Nginx config**: Custom site config at `/etc/nginx/sites-available/bi-analysis`. PHP frontend on port 80, Python API proxied on port 8001 to uWSGI at 127.0.0.1:7625.
